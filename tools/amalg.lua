...
local function is_bytecode(path)
	local f <close>, res = io.open(path, "rb"), false
	if f then
		res = f:read(1) == "\027"
	end
	return res
...
local function readfile(path, is_bin)
	local f <close> = assert(io.open(path, is_bin and "rb" or "r"))
...
local function writecache(c)
	local f <close> = assert(io.open(cache, "w"))
	f:write"return {\n"
...
local dllnames = {}
  ...
  local f <close> = assert( io_open( dll, "wb" ) )
  f:write( ]=], qcode, [=[ )
