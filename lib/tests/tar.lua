...
test["extract"] = function ()
    ...
    local ok, err = eli_tar.safe_extract("assets/test.tar", "tmp")
    test.assert(ok, err)
    local ok, file_hash = eli_fs.safe_hash_file("tmp/f1.txt", { hex = true })
    ...
    local ok, file_hash = eli_fs.safe_hash_file("tmp/f2.txt", { hex = true })
    test.assert(ok, file_hash)
    test.assert(file_hash == "76f60fe07ed3fac881ff3022b141f1cd56242152a43fa90b1a3ac423a2efc7ba", "hashes dont match")
    local ok, file_hash = eli_fs.safe_hash_file("tmp/f3.txt", { hex = true })
    test.assert(ok, file_hash)
...
test["extract_file"] = function ()
    ...
    local ok, err = eli_tar.safe_extract_file("assets/test.tar", "f2.txt", "tmp/f2.txt.untared")
    test.assert(ok, err)
    local ok, file_hash = eli_fs.safe_hash_file("tmp/f2.txt.untared", { hex = true })
    test.assert(ok, file_hash)
    test.assert(file_hash == "76f60fe07ed3fac881ff3022b141f1cd56242152a43fa90b1a3ac423a2efc7ba", "hashes dont match")
end

test["extract_string"] = function ()
    local ok, file = eli_tar.safe_extract_string("assets/test.tar", "f3.txt")
    test.assert(ok, file)
    local ok, file_hash = eli_hash.safe_sha256_sum(file, true)
    test.assert(ok, file_hash)
    test.assert(file_hash == "7e5449fc89e75e0b8c6cbb3568720d074fa435f88dd9bf5e8b82c012a6c86c2a", "hashes dont match")
end
