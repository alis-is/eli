...
_test["copy file"] = function ()
    local _ok, _error = _eliFs.safe_copy_file("assets/test.file", "tmp/test.file")
    _test.assert(_ok, _error)
    local _ok, _hash = _eliFs.safe_hash_file("assets/test.file", {type = "sha256"})
...
_test["remove (contentOnly)"] = function ()
    local _ok, _error = _eliFs.safe_mkdir("tmp/test-dir")
    _test.assert(_ok, _error)
    local _ok, _error = _eliFs.safe_copy_file("assets/test.file", "tmp/test-dir/test.file")
    _test.assert(_ok, _error)
